...
function edgeWork(radius) {
    gl.edgeWork1 = gl.edgeWork1 || new Shader(null, '\
        ...
        void main() {\
            ...
            gl_FragColor = vec4(total / color, 0.0, 0.5);\
            \
            /* total / color vs color / total */\
        }\
    ');
    gl.edgeWork2 = gl.edgeWork2 || new Shader(null, '\
        ...
        void main() {\
            ...
            float c = clamp(100.0 * (color.y / total.y - color.x / total.x) + 0.35, 0.0, 0.25);\
            gl_FragColor = vec4(c, c, c, 0.5);\
            \
            /* changed so that color can come from through via CSS */\
        }\
    ');
